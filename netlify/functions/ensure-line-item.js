<!-- =============================
     HSRESULTS (FULL DROP-IN V4.1 - UPLOAD DEBUG + STUCK-FLAG SAFE)
     ...
     ✅ Replaces #downloadPDF with #downloadPDF_exec so old scripts can't hijack click
     ✅ Adds upload state machine so mobile/stale flags don't skip upload
     ✅ Redirects to http://www.homesecurecalculator.com/hscthankyou
     ✅ Debug: add &debug=1 to URL
     ✅ Force upload: add &force_upload=1 to URL
============================= -->
...
<div style="font-family:Inter,Segoe UI,sans-serif;background:#f4f6fb;padding:60px 20px;min-height:100vh;">
  <div style="max-width:1500px;margin:auto;display:grid;grid-template-columns:1fr 1fr;gap:40px;align-items:start;">
    ...
    <div style="background:#ffffff;border-radius:18px;padding:40px;border:1px solid #e5e7eb;box-shadow:0 20px 60px rgba(0,0,0,.06);display:flex;flex-direction:column;gap:18px;">
      ...
      <!-- Old id stays, but we will replace with #downloadPDF_exec -->
...
<script>
(() => {
  if (window.__HSC_RESULTS_FULL_DROPIN_V4_1__) return;
  window.__HSC_RESULTS_FULL_DROPIN_V4_1__ = true;

  // disable any old guard flags that might be checked by earlier scripts
  window.__HSC_RESULTS_UPLOAD_V4__ = true;
  window.__HSC_HSRESULTS_EXEC_V1__ = true;
  ...
  const INFLOW_LOCK    = "hsc_results_continue_lock_exec_v4_1";
  const INFLOW_LOCK_TS = INFLOW_LOCK + "_ts";

  const byId = (id) => document.getElementById(id);
  const safeText = (id, v) => { const n = byId(id); if(n) n.textContent = String(v ?? ""); };
  const safeHTML = (id, v) => { const n = byId(id); if(n) n.innerHTML = String(v ?? ""); };
  ...
  const FORCE_UPLOAD = qp("force_upload") === "1";

  function log(...args){ if(DEBUG) console.log("[HSRESULTS]", ...args); }
  function warn(...args){ console.warn("[HSRESULTS]", ...args); }
  ...
  function hydratePayload(payload){
    ...
    const qLead  = String(qp("lead_id")||"").trim();
    const qEmail = String(qp("email")||"").trim();
    const qDeal  = String(qp("deal_id")||"").trim();
  ...
  function buildDeviceLines(payload){
    ...
    return parseSelectedItems(payload.selectedItems || payload.hsc_devices || payload.deviceSummary || "")
      .filter(x => x.qty > 0);
  }
  ...
  async function uploadDeliverables(leadId, dealId, pdfB64, csvText){
    const payload = { lead_id: leadId, deal_id: dealId, pdf_base64: pdfB64, csv_text: csvText };
    log("POST upload-deliverables payload sizes", {
      pdf_base64_len: (pdfB64||"").length,
      csv_len: (csvText||"").length,
      lead_id: leadId,
      deal_id: dealId
    });

    const res = await fetchJsonWithFallback([UPLOAD_ENDPOINT], {
      method:"POST",
      headers:{ "Content-Type":"application/json" },
      body: JSON.stringify(payload)
    });

    // persist last response for debugging (even on mobile)
    try { sessionStorage.setItem("hsc_last_upload_response", JSON.stringify(res)); } catch(e){}
    log("upload-deliverables response", res);
  ...
  function hardRedirect(leadId, dealId, email){
    const u = new URL(THANKYOU_URL);
    u.searchParams.set("lead_id", leadId);
    u.searchParams.set("deal_id", dealId);
    if(email) u.searchParams.set("email", email);
    window.location.href = u.toString();
  }

  function getActionBtn(){
    return byId("downloadPDF_exec") || byId("downloadPDF") || null;
  }

  // ✅ per-lead+deal upload state machine
  function uploadStateKey(leadId, dealId){
    return "hsc_upload_state_v1_" + String(leadId||"") + "_" + String(dealId||"");
  }
  function getUploadState(leadId, dealId){
    const k = uploadStateKey(leadId, dealId);
    try{
      const raw = sessionStorage.getItem(k);
      return raw ? JSON.parse(raw) : null;
    }catch(e){ return null; }
  }
  function setUploadState(leadId, dealId, state){
    const k = uploadStateKey(leadId, dealId);
    try{ sessionStorage.setItem(k, JSON.stringify(state || {})); }catch(e){}
  }
  function clearUploadState(leadId, dealId){
    const k = uploadStateKey(leadId, dealId);
    try{ sessionStorage.removeItem(k); }catch(e){}
  }

  async function onContinue(){
    console.info("[HSRESULTS] onContinue() fired");

    // auto-clear stuck lock after 20 seconds
    const lock = getStoredAny(INFLOW_LOCK);
    const ts = Number(getStoredAny(INFLOW_LOCK_TS) || "0");
    if(lock === "1" && ts && (Date.now() - ts) > 20000){
      clearStored(INFLOW_LOCK);
      clearStored(INFLOW_LOCK_TS);
    }
    if(getStoredAny(INFLOW_LOCK) === "1"){
      warn("blocked by inflow lock");
      return;
    }
    setStored(INFLOW_LOCK, "1");
    setStored(INFLOW_LOCK_TS, String(Date.now()));

    const btn = getActionBtn();
    if(btn){
      btn.disabled = true;
    ...
    try{
      ...
      // Ensure canvases have pixels for toDataURL
      ...
      if(!payload.lead_id || !payload.email){
        ...
        throw new Error("Missing lead_id/email.");
      }
      ...
      // state logic (prevents “upload skipped” on mobile)
      const state = getUploadState(leadId, dealId);
      const now = Date.now();

      // if in_progress is older than 3 min -> clear (mobile tab sleep)
      if(state?.status === "in_progress" && state?.ts && (now - state.ts) > 180000){
        warn("clearing stale upload in_progress state");
        clearUploadState(leadId, dealId);
      }

      const state2 = getUploadState(leadId, dealId);
      const alreadyDone = (state2?.status === "done");
      const shouldUpload = FORCE_UPLOAD ? true : !alreadyDone;

      log("upload state", { state: state2, shouldUpload, FORCE_UPLOAD });

      if(shouldUpload){
        setUploadState(leadId, dealId, { status:"in_progress", ts: now });

        safeText("statusText","Working…");
        safeText("statusSub","Generating Executive PDF + CSV…");

        const pdfBlob = await buildExecutivePdfBlob(payload);
        ...
        if(!up.ok){
          setUploadState(leadId, dealId, { status:"failed", ts: Date.now(), error: up.error || up.text || "Upload failed" });
          safeText("statusText","Error");
        ...
        // ✅ only mark done AFTER success
        setUploadState(leadId, dealId, { status:"done", ts: Date.now(), result: up.json || null });
      } else {
        safeText("statusText","Ready");
        safeText("statusSub","Deliverables already uploaded. Redirecting…");
      }

      safeText("statusText","Redirecting…");
      safeText("statusSub","Opening your PDF preview…");
      return hardRedirect(leadId, dealId, payload.email || "");

    } catch(err){
      console.error("[HSRESULTS] Continue failed:", err);
      clearStored(INFLOW_LOCK);
      clearStored(INFLOW_LOCK_TS);
      const btn2 = getActionBtn();
      if(btn2){ btn2.disabled=false; btn2.style.opacity="1"; btn2.style.cursor="pointer"; }
    }
  }

  // ✅ Replace the button id so old scripts cannot match it
  function replaceButton(){
    const oldBtn = byId("downloadPDF");
    if(!oldBtn || !oldBtn.parentNode) return;

    oldBtn.disabled = true;
    oldBtn.style.display = "none";
    oldBtn.id = "downloadPDF_old";

    const newBtn = document.createElement("button");
    newBtn.id = "downloadPDF_exec";
    newBtn.textContent = "Continue to Download PDF →";
    newBtn.setAttribute("type","button");
    newBtn.style.cssText = "padding:14px;border:none;border-radius:14px;background:#111827;color:#fff;font-weight:900;cursor:pointer;width:100%;";
    ...
    console.info("[HSRESULTS] replaced #downloadPDF with #downloadPDF_exec");
  }

  document.addEventListener("DOMContentLoaded", async () => {
    replaceButton();
    console.info("[HSRESULTS] Full Drop-In V4.1 loaded (exec PDF + upload + redirect).");
