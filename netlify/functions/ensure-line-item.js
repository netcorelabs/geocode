  ...
  async function getAssocTypeIdDealsLineItems() {
    try {
      ...
      if (r.ok && r.json?.results?.length) {
        console.log("Association labels from HubSpot:", r.json.results);
      ...
      console.warn("Failed to fetch association labels:", e);
    }

    // Fallback: use HubSpot’s default for deals→line_items
    console.warn("Using fallback associationTypeId 6");
    return 6;
  }
